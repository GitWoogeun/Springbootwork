...
메인페이지
    ...
    {                                                                         // MyBatis라면
        작성자 : rnb                          => username = [ User 테이블 ]     3개를 Join해서
        제목   : 오늘은 연관관계 주인 배우는 날 => title    = [ Board 테이블 ]    Select해서
        내용   : 어렵다                       => content  = [ Board 테이블 ]    컬럼을 가져왔을 것
        댓글                                 => content  = [ Reply 테이블 ]
            ssar : 맞아요!                                                    // ORM 방식이라면
            cos  : 저도 배우고 있어요!                                         Board 테이블만 
    }                                                                        Select * From Board
                                                                             where Id = 1;
                        자바프로그램  =>  JPA  =>  DB
